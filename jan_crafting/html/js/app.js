...
var RotationDegree = 1700*(RotationTime/15000);
...
(() => {
    ...
    CraftGame.Close = function() {
        ...
        $.post("http://jan_crafting/CloseGame", JSON.stringify({}));
    };

    CraftGame.Failed = function() {
        if (active) $.post("http://jan_crafting/CraftingFailed",'{}');
        active = false;
        CraftGame.Close();
    }

    CraftGame.Success = function() {
        if (active) $.post("http://jan_crafting/CraftingSuccess",'{}');
        active = false;
        CraftGame.Close();
    }
